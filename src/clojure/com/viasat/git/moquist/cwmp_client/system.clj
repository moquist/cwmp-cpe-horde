(ns com.viasat.git.moquist.cwmp-client.system
  (:require
   [clojure.edn :as edn]
...
(defn load-config
  ...
  (let [config (edn/read-string (slurp config-file-path))]
    (when-not (s/valid? :com.viasat.git.moquist.cwmp-client.system/config config)
      (throw (ex-info (format "config file at %s is not valid" config-file-path)
                      (s/explain-data :com.viasat.git.moquist.cwmp-client.system/config config))))
    config))
